...
class ConvLayerDownsampleLayer(Layer_01):
    """
    ...
    def __init__(self, numfilters1_in,   numfilters1_out,  bias_, L1 = 0., L2 = 0.,device = None):
        ...
        """
        super(ConvLayerDownsampleLayer, self).__init__()
...
class AtrousPyramidLayer(Layer_01):
    """
    ...
    def __init__(self,num_filteres_in=3, num_filteres_out=1,num_filteres_middle=1, L1 = 0., L2 = 0.,device = None,show=0):
        ...
        На вход подаются следующие параметры:
         ...
         :param device - Выбор CPU/GPU(:cuda)
        """
        super(AtrousPyramidLayer, self).__init__()
        self.class_name = self.__class__.__name__        
        if (device is not None):
            self.device = device if (not isinstance(device, str)) else torch.device(device)
        else:
...
class ConvLayerUniversalUpsampleLayer(Layer_06):
    """
    ...
    def __init__(self, numfilters_in, numfilters_out, k_size, bias_, L1 = 0., L2 = 0., device = None):
        ...
        На вход подаются следующие параметры:
         ...
         :param device - Выбор CPU/GPU(:cuda)
        """        
        super(ConvLayerUniversalUpsampleLayer, self).__init__()
...
class FunOfFactorLayer(Layer_01):
    """
    ...
    def __init__(self, Size_,z1,z2,z3,z4,z5,  last_activate,  device = None):
        ...
        На вход подаются следующие параметры:
         ...
         :param device - Выбор CPU/GPU(:cuda)
        """          
        super(FunOfFactorLayer, self).__init__()
        self.Size = Size_ 
...
class FullyConvUniversalLayer(Layer_01):
    """
    ...
    def __init__(self, Size_, last_activate, device = None):
        ...
        На вход подаются следующие параметры:
         ...
         :param device - Выбор CPU/GPU(:cuda)
        """          
        super(FullyConvUniversalLayer, self).__init__()
        self.Size = Size_[0]
        self.last_activate=last_activate
        self.class_name = self.__class__.__name__
...
class SegmentationWireModel1(Layer_06):
    """
    ...
    def __init__(self,
        ...
        super(SegmentationWireModel1, self).__init__((imageSize[0], imageSize[1], 3))
        ...
        num_filteres_out = self.dict_layers['num_filteres_out']
        num_filteres_middle = self.dict_layers['num_filteres_middle']
        num_filteres_in = self.dict_layers['num_filteres_in']
        
        self.atrous_pyramid_00 = AtrousPyramidLayer(num_filteres_in, num_filteres_out, num_filteres_middle, self.L1, self.L2, self.device, self.show)
       
        self.atrous_pyramid_01 = AtrousPyramidLayer(self.dict_layers['atrous_pyramid_01'][0],
                                                    self.dict_layers['atrous_pyramid_01'][1],
                                                    self.dict_layers['atrous_pyramid_01'][2],
                                                    self.L1, self.L2, self.device, self.show)

        self.atrous_pyramid_02 = AtrousPyramidLayer(self.dict_layers['atrous_pyramid_02'][0],
                                                    self.dict_layers['atrous_pyramid_02'][1],
                                                    self.dict_layers['atrous_pyramid_02'][2],
                                                    self.L1, self.L2, self.device, self.show)
        ...
        self.add_module('fully_conn_layer_universal_00', FullyConvUniversalLayer([624, 128, 128, 64, 64, 64], 'sigmoid', self.device))
        ...
        self.add_module('fun_of_factor_00', FunOfFactorLayer(32, 24, 12, 12, 12, 3, 'sigmoid', self.device))
        ...
        self.conv_layer_universal_01_downsampl=ConvLayerDownsampleLayer(self.dict_layers['conv_layer_DS_01'][0],
                                                                        self.dict_layers['conv_layer_DS_01'][1],
                                                                        True, self.L1, self.L2, self.device)

        self.conv_layer_universal_02_downsampl=ConvLayerDownsampleLayer(self.dict_layers['conv_layer_DS_02'][0],
                                                                        self.dict_layers['conv_layer_DS_02'][1],
                                                                        True, self.L1, self.L2, self.device)

        self.conv_layer_universal_03_downsampl=ConvLayerDownsampleLayer(self.dict_layers['conv_layer_DS_03'][0],
                                                                        self.dict_layers['conv_layer_DS_03'][1],
                                                                        True, self.L1, self.L2, self.device)

        self.conv_layer_universal_04_downsampl=ConvLayerDownsampleLayer(self.dict_layers['conv_layer_DS_04'][0],
                                                                        self.dict_layers['conv_layer_DS_04'][1],
                                                                        True, self.L1, self.L2, self.device)

        # Четыре ДеКонволюционных слоя с уменьшением каналов изображения
        self.conv_layer_universal_01_upsampl=ConvLayerUniversalUpsampleLayer(self.dict_layers['conv_layer_UP_01'][0],
                                                                             self.dict_layers['conv_layer_UP_01'][1],
                                                                             self.dict_layers['conv_layer_UP_01'][2],
                                                                             True, self.L1, self.L2, self.device)

        self.conv_layer_universal_02_upsampl=ConvLayerUniversalUpsampleLayer(self.dict_layers['conv_layer_UP_02'][0],
                                                                             self.dict_layers['conv_layer_UP_02'][1],
                                                                             self.dict_layers['conv_layer_UP_02'][2],
                                                                             True, self.L1, self.L2, self.device)

        self.conv_layer_universal_03_upsampl=ConvLayerUniversalUpsampleLayer(self.dict_layers['conv_layer_UP_03'][0],
                                                                             self.dict_layers['conv_layer_UP_03'][1],
                                                                             self.dict_layers['conv_layer_UP_03'][2],
                                                                             True, self.L1, self.L2, self.device)

        self.conv_layer_universal_04_upsampl=ConvLayerUniversalUpsampleLayer(self.dict_layers['conv_layer_UP_04'][0],
                                                                             self.dict_layers['conv_layer_UP_04'][1],
                                                                             self.dict_layers['conv_layer_UP_04'][2],
                                                                             True, self.L1, self.L2, self.device)
          
         
        # Конволюционный слой из Конвалюции+БатчНормализации+Функция активации. 
        self.add_module('conv_layer_universal_uno_1', conv_layer_universal_uno_04(self.dict_layers['conv_layer_UNO_01'][0],
                                                                                  self.dict_layers['conv_layer_UNO_01'][1],
                                                                                  1,
                                                                                  'relu',
                                                                                  self.dict_layers['conv_layer_UNO_01'][2],
                                                                                  2,
                                                                                  self.device))

        self.add_module('conv_layer_universal_uno_2', conv_layer_universal_uno_04(self.dict_layers['conv_layer_UNO_02'][0],
                                                                                  self.dict_layers['conv_layer_UNO_02'][1],
                                                                                  1,
                                                                                  'relu',
                                                                                  self.dict_layers['conv_layer_UNO_02'][2],
                                                                                  0,
                                                                                  self.device))

        self.add_module('conv_layer_universal_uno_3', conv_layer_universal_uno_04(self.dict_layers['conv_layer_UNO_03'][0],
                                                                                  self.numclasses,
                                                                                  1,
                                                                                  'linear',
                                                                                  self.dict_layers['conv_layer_UNO_03'][1],
                                                                                  0,
                                                                                  self.device))
        ...
        self.encoder_00_upsampl=ConvLayerUniversalUpsampleLayer(self.dict_layers['conv_layer_encoder_01'][0],
                                                                self.dict_layers['conv_layer_encoder_01'][1],
                                                                self.dict_layers['conv_layer_encoder_01'][2],
                                                                True, self.L1, self.L2, self.device)

        # Конволюционный слой с увелечением колличества каналов изображения
        _encoder_map=ConvLayerDownsampleLayer(self.dict_layers['encoder_map_01'][0],
                                              self.dict_layers['encoder_map_01'][1],
                                              True, self.L1, self.L2, self.device)

        self.add_module('encoder_map',_encoder_map) 

        # Перемешивание вдоль каналов в разреженных пирамидах
        _layer_conv_D0 = Conv2d(self.dict_layers['layer_D0'][0],
                                self.dict_layers['layer_D0'][1],
                                kernel_size=(1, 1), stride=(1, 1), padding_mode = 'zeros', bias=True)
                                
        _layer_conv_D1 = Conv2d(self.dict_layers['layer_D1'][0],
                                self.dict_layers['layer_D1'][1],
                                kernel_size=(1, 1), stride=(1, 1), padding_mode = 'zeros', bias=True)

        _layer_conv_D2 = Conv2d(self.dict_layers['layer_D2'][0],
                                self.dict_layers['layer_D2'][1],
                                kernel_size=(1, 1), stride=(1, 1), padding_mode = 'zeros', bias=True)
	...
	# применение модели. на вход- 3 канальный имедж от 0 до 1 стандартного размера, задаваемое параметрами класса
        ...
        merged_01 = torch.cat((self.dict_layers['torch_cat1'][0] * im_02_upsmpl,
                               self.dict_layers['torch_cat1'][1] * im_02_dwnsmpl),
                               axis=1) 
        ...
        merged_02 = torch.cat((self.dict_layers['torch_cat2'][0] * im_04_upsmpl,
                               self.dict_layers['torch_cat2'][1] * im_0),
                               axis=1)
        if self.show:
    ...
    def fit_dataloader_00(self, loader,  epochs = 1, validation_loader = None):
        ...
        return history
#№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№№
class SegmentationWireModel2(Layer_06):
    def __init__(self, imageSize,  last_activate, L1 = 0., L2 = 0.,device = None,numclasses=10 ):
        super(SegmentationWireModel2, self).__init__((imageSize[0], imageSize[1], 3))
        ...
        self.atrous_pyramid_00 = AtrousPyramidLayer\
        (num_filteres_in , num_filteres_out ,num_filteres_middle, self.L1, self.L2, self.device,self.show)
       
        self.atrous_pyramid_01 = AtrousPyramidLayer\
        (3 , 24 ,12, self.L1, self.L2, self.device,self.show)
        self.atrous_pyramid_02 = AtrousPyramidLayer\
        (42 , 64 ,24, self.L1, self.L2, self.device,self.show)
 
        ###############
        self.add_module('fully_conn_layer_universal_00', \
                        FullyConvUniversalLayer([624, 128, 128, 64, 64, 64], 'sigmoid', self.device))
        self.add_module('fun_of_factor_00', \
                        FunOfFactorLayer(32, 24, 12, 12, 12, 3, 'sigmoid', self.device))
        if 0:
            self.im_to_32_features_00=im_to_32_features_01(L1, L2, self.device)
            self.sketch_to_64_features_00=sketch_to_64_features_01(L1, L2, self.device)
        ###
        self.conv_layer_universal_01_downsampl=ConvLayerDownsampleLayer(24, 32, True, self.L1, self.L2, self.device)
        self.conv_layer_universal_02_downsampl=ConvLayerDownsampleLayer(32, 42, True, self.L1, self.L2, self.device)
        
        
        
        self.conv_layer_universal_03_downsampl=ConvLayerDownsampleLayer(64, 86, True, self.L1, self.L2, self.device)
        self.conv_layer_universal_04_downsampl=ConvLayerDownsampleLayer(86, 124, True, self.L1, self.L2, self.device)
        ###########3
        
        self.conv_layer_universal_01_upsampl=ConvLayerUniversalUpsampleLayer(64, 48, 3, True, self.L1, self.L2, self.device)
        self.conv_layer_universal_02_upsampl=ConvLayerUniversalUpsampleLayer(48, 32, 3, True, self.L1, self.L2, self.device)
        self.conv_layer_universal_03_upsampl=ConvLayerUniversalUpsampleLayer(32 + 42, 32, 3, True, self.L1, self.L2, self.device)
        self.conv_layer_universal_04_upsampl=ConvLayerUniversalUpsampleLayer(32, 24, 3, True, self.L1, self.L2, self.device)
        ############## 
        ...
        self.add_module('Sgmd', _layer_Sgmd)  
        #################################################33
        self.encoder_00_upsampl=ConvLayerUniversalUpsampleLayer(128, 64, 3, True, self.L1, self.L2, self.device)
        _encoder_map=ConvLayerDownsampleLayer(64, 128, True, self.L1, self.L2, self.device)
        self.add_module('encoder_map',_encoder_map) 
    ...
    def forward(self, im_wire):
        ...
        if self.show:
            print('im_04_dwnsmpl',im_04_dwnsmpl.shape)
        ##############################
        atrous_pyramid_= self.atrous_pyramid_00(im_04_dwnsmpl)
        if self.show:
            print('atrous_pyramid_ ',atrous_pyramid_.shape)
         
         
        atrous_pyramid_00=self.encoder_map(atrous_pyramid_)
        ...
        #merged_02 = torch.cat((im_0,im_0 ),axis=1)
        if self.show:
            print(' merged_02', merged_02.shape)
...
#                return -torch.lloss_batch_00og(torch.mean(torch.sum(soft_targets * pred, 1)))
...
#                _, yb = torch.max(yb, dim = 1)
        ...
        def loss_batch_02(loss_func, xb, yb, opt=None):
#            def cross_entropy(pred, soft_targets):
#                return -torch. loss_batch_02log(torch.mean(torch.sum(soft_targets * pred, 1)))
                #logsoftmax = nn.LogSoftmax()
                #return torch.pow(1 - torch.mean(torch.sum(soft_targets * pred, 1)), 2)
                #return torch.mean(torch.sum(- soft_targets * torch.norm(pred, p=2,dim=1), 1))
                #return torch.mean(torch.sum(- soft_targets * logsoftmax(pred), 1))
...
#                _, yb = torch.max(yb, dim = 1)
        ...
        for epoch in range(epochs):
            ...
            for s in loader:
                ...
                im_to_segm=train_ds.tensors[0]# 
                 
                target=train_ds.tensors[1]
        ...
        return history
###########################################
    def fit_dataloader_01(self, W_, loader,  epochs = 1, validation_loader = None):
        
        W_=W_[:7] 
        W_=np.array(W_)
        W_=W_/np.sum(W_)
        W_=torch.FloatTensor(W_).to(self.device) 
...
#        _optimizer = optim.SGD(self.parameters(), lr=0.001, momentum=0.2)
        ...
        def loss_batch_00(W_,loss_func, xb, yb, opt=None):
#            def cross_entropy(pred, soft_targets):
#                return -torch.lloss_batch_00og(torch.mean(torch.sum(soft_targets * pred, 1)))
                #logsoftmax = nn.LogSoftmax()
                #return torch.pow(1 - torch.mean(torch.sum(soft_targets * pred, 1)), 2)
...
#                _, yb = torch.max(yb, dim = 1)
            ...
            if self.flag0986556 == 0:
                
                loss = loss_func(pred0*W_, yb*W_)
            else:
                loss = loss_func(pred0, yb)
        ...
        def loss_batch_02(W_,loss_func, xb, yb, opt=None):
#            def cross_entropy(pred, soft_targets):
#                return -torch. loss_batch_02log(torch.mean(torch.sum(soft_targets * pred, 1)))
                #logsoftmax = nn.LogSoftmax()
                #return torch.pow(1 - torch.mean(torch.sum(soft_targets * pred, 1)), 2)
...
#                _, yb = torch.max(yb, dim = 1)
        ...
        for epoch in range(epochs):
            ...
            for s in loader:
                ...
                losses_, nums_ ,acc_  =   loss_batch_02(W_, self._criterion, \
                                                   ( im_to_segm ),\
                                                   target,  self._optimizer)                     
            ...
            if validation_loader is not None:
                ...
                #s = next(iter(loader))
                    ...
                    losses_, nums_ ,acc_  =   loss_batch_00(W_, self._criterion, \
                                                       ( im_to_segm ),\
                                                       target,  self._optimizer)                                                                                                       
        ...
        return history
    ############################################################################################
    def fit_dataloader_05_maska_md(self, loader,  epochs = 1, validation_loader = None):
...
#        _optimizer = optim.SGD(self.parameters(), lr=0.001, momentum=0.2)
        ...
        def loss_batch_02( loss_func, xb, yb,mask, opt=None):
#            def cross_entropy(pred, soft_targets):
#                return -torch. loss_batch_02log(torch.mean(torch.sum(soft_targets * pred, 1)))
                #logsoftmax = nn.LogSoftmax()
                #return torch.pow(1 - torch.mean(torch.sum(soft_targets * pred, 1)), 2)
...
#                _, yb = torch.max(yb, dim = 1)
            ...
            #print(pred0.shape,yb.shape,mask_.shape) 
            f0= torch.mul(pred0,1-mask_)
            f1= torch.mul(yb,1-mask_)
            g0= torch.mul(pred0, mask_)
            g1= torch.mul(yb, mask_)

            #print(f0.shape)
            loss = loss_func(f0, f1)+0.01*loss_func(g0, g1)
        ...
        for epoch in range(epochs):
            ...
            for s in loader:
                s1= torch.FloatTensor(s['Anchor'].numpy()).to(self.device)
                s2 =torch.FloatTensor(s['label'].numpy()).to(self.device)
                s3=torch.FloatTensor(s['mask_md'].numpy()).to(self.device)
            
                train_ds = TensorDataset(s1,s2,s3)                                  
